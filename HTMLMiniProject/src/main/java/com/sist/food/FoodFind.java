...
public class FoodFind extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1. 브라우저로 전송 => HTML을 전송한다 
			    ...
			    List<FoodVO> list=dao.foodFind(curpage, colum, fd);
			    System.out.println(list.size());
			    int totalpage=dao.foodFindTotalPage(colum, fd);
				...
				out.println("<button class=\"btn btn-sm btn-danger\">검색</button>");
				out.println("</form>");
				out.println("</div>");
				...
				if(startPage>1)
				{
				  out.println("<li><a href=\"MainServlet?mode=4&colum="+colum+"&fd="+fd+"&page="+(startPage-1)+"\">&lt;</a></li>");
				}
				
				for(int i=startPage;i<=endPage;i++)
				{
				 if(i==curpage)
				  out.println("<li class=active><a href=\"MainServlet?mode=4&colum="+colum+"&fd="+fd+"&page="+i+"\">"+i+"</a></li>");
				 else
				  out.println("<li><a href=\"MainServlet?mode=4&colum="+colum+"&fd"+fd+"&page="+i+"\">"+i+"</a></li>");
				}
				
				if(endPage<totalpage)
				{
				  out.println("<li><a href=\"MainServlet?mode=4&colum="+colum+"&fd"+fd+"&page="+(endPage+1)+"\">&gt;</a></li>");
				}
				out.println("</ul>");
