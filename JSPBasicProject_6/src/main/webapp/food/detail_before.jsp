...
<%--
	Cookie => 301page
	HTTP 프로토콜은 새로운 페이지가 실행되면 기존에 입력된 모든 정보가 사라진다
	----------- 로그인 -> 새로운페이지로 이동 -> 로그인정보 사라진다
				=> 지속성(유지성)이 없다
				   ----------------> 정보가 계속 유지할 수 있게 만든 라이브러리 
				   						: Session / Cookie
				   						  ~~~~~~~~~~~~~~~~~ 예약정보, 장바구니
	 => 상태의 지속성(Cookie / Session)
	    --- 데이터의 변경 상황
	    React / Vue => state관리 프로그램(데이터상태 프로그램)
	 ...
	 => 사용법
	 	...
	 	4. 저장기간 지정 : setMaxAge(초단위) = 60*60*24 => default: 30분
	 	5. 저장된 데이터 읽기
	 		-> 키 읽기 : getName()
	 		-> 값 읽기 : getValue()
	 	6. 저장 경로 설정
		...
		9. 자동 로그인 (쿠키는 가능 / 세션은 불가능)
			쿠키는 브라우저에 저장되어 있기 때뮨애 저장기간동안 데이터 유지
			=> 자동 로그인이 가능하다 
			==> Spring (remember-me)				
