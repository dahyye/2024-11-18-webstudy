...
public class ReplyDAO {
	...
	<select id="replyInsert" parameterType="ReplyVO" >
	 INSERT INTO all_comment(cno,rno,type,id,name,sex,msg,group_id) 
	 VALUES((SELECT NVL(MAX(cno)+1,1) FROM all_comment),#{rno}, #{cno},#{type},
	 			#{id},#{name},#{sex},#{msg},(SELECT NVL(MAX(group_id)+1,1) FROM all_comment)
	</select>
	*/
